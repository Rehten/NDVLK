...
export interface FeatureComponent<T extends VirtualMetadata> extends VirtualComponent<T> {
  // должен содержать все компоненты, что задекларированы(не импортированы) в этом модуле.
  readonly $$complexFactoryMap: Map<string,
    VirtualComplexFactory<VirtualComplex<VirtualComponent<VirtualMetadata>, VirtualMetadata>,
      VirtualComponent<VirtualMetadata>,
      VirtualMetadata>>;
